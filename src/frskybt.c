...
void logBTFrame(bool valid, char message[]) {
  ...
  if (!valid) {
    ESP_LOGE(FRSKYBT_TAG, "(%05lld)Unable to decode data, %s", timestamp,
             message);
  } else {
    ESP_LOGI(FRSKYBT_TAG,
             "(%05lld)Ch1[%04d] Ch2[%04d] Ch3[%04d] Ch4[%04d] Ch5[%04d] "
             "Ch6[%04d] Ch7[%04d] Ch8[%04d]",
             timestamp, frchanneldata[0], frchanneldata[1], frchanneldata[2],
             frchanneldata[3], frchanneldata[4], frchanneldata[5],
             frchanneldata[6], frchanneldata[7]);
  }
}
...
void processTrainerFrame(const uint8_t *otxbuffer) {
  for (uint8_t channel = 0, i = 1; channel < BT_CHANNELS;
       channel += 2, i += 3) {
    // +-500 != 512, but close enough.
    frchanneldata[channel] = otxbuffer[i] + ((otxbuffer[i + 1] & 0xf0) << 4);
    frchanneldata[channel + 1] = ((otxbuffer[i + 1] & 0x0f) << 4) +
                                 ((otxbuffer[i + 2] & 0xf0) >> 4) +
                                 ((otxbuffer[i + 2] & 0x0f) << 8);
  }

  // If the data came from the radio, send it out over bluetooth. Send same data
  // but add the START_STOP
...
void frSkyProcessByte(uint8_t data) {
  ...
  case STATE_DATA_XOR:
    ...
    case START_STOP ^ STUFF_MASK:
      ...
      dataState = STATE_DATA_IN_FRAME;
      break;
    case START_STOP: // Illegal situation, as we have START_STOP, try to start
                     // from the beginning
      otxbufferIndex = 0;
      dataState = STATE_DATA_IN_FRAME;
      break;
