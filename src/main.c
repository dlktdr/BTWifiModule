...
/* Service */
static const uint16_t GATTS_SERVICE_UUID_TEST = 0xFFF0;
static const uint16_t GATTS_CHAR_UUID_TEST_A = 0xFFF6;
    ...
    case ESP_GATTS_REG_EVT: {
      ...
      if (raw_adv_ret) {
        ESP_LOGE(GATTS_TABLE_TAG,
                 "config raw adv data failed, error code = %x ", raw_adv_ret);
      }
      adv_config_done |= ADV_CONFIG_FLAG;
...
void runBlinky() {
  int i = 33;
  gpio_set_direction(i, GPIO_MODE_DEF_OUTPUT);
  while (1) {
